...
//задание сделать форму для заполнения с проверкой данныхю универсальная для ссайтов
...
errorMessage.innerHTML = "";
 ...
 if(!password.value.trim() || !isStrongPassword(password.value)){
  displayError("пароль должен состоять как минимум из 8 символов, как минимум 1 заглавную букву, 1 строчную букву, 1 цифру и 1 специальный символ");
 }

 alert("Registr successfull");
 registrationForm.reset();
})

function displayError(massage){
errorMessage.innerHTML += `<div class='error'>${massage}`;
}
...
// новая тема . работа с обьектами
//создание обьектов  2 способа
//let car = new Object();
//let car1 = {};
...
// car["type-brand"] = "BMW"
// car["color"] =  "white"
//ключ и значение . выводится только в консоли
...
//только так можно вывести на экран или в алерт
//мы можем получать данные сторонние(дефиси пробел )

//второй способ создания обьектов
...
// menu1.title = "menu";
// console.log(menu1);
// //в консоли показывается в алфавитном порядке ключи
...
//console.log("width" in munu);

//циклы

// for(let key in menu){
//   document.writeln("<br> ключ =" + key + ", значение= " + menu[key])
//   //в цикле нужны [] в document
// }

//добавление какого-то свойства

//menu.age = 25;

//document.writeln(menu.length)
//length не существует у document
//можно через переменную увеличивающуюся найти . например через  let count = 0; count++;(в функции)
// // =============================================
//находим имена ключей
// Object.keys(menu)
...
// к массиву можно обратиться через map  но для другого объекта это не возможно
let mas3 = Object.keys(car.colors).map(function (elem) {
	return elem + ": " + car.colors[elem]
})
document.writeln('<br>' + mas3 + '<br>') */
...
//ищем и вызываем свойство через this */
...
//   }
// }
// новый формат записи. ключ и значение одинаковые
let coord = {x,y,
	calcsq() {
		document.writeln(this.x * this.y)
	}
